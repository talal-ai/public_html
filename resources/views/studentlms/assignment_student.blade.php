                        ...
                        <tbody>
                            <template x-for="(item, index) in items" :key="index" x-data="myComponent()">
                                <tr x-show="checkView(index + 1)" class="">
                                    ...
                                    <td>
                                        <div x-show="item.obtainMarks !== null"
                                            class="rounded p-3 bg-success/10 text-success mt-3 text-center">
                                            <span x-text="item.obtainMarks"></span>
                                        </div>
                                        <div x-show="item.obtainMarks === null"
                                    ...
                                    <td>
                                        <div x-show="item.assignmentsubmitbystudnet === 1"
                                            class="rounded p-3 bg-success/10 text-success mt-3 text-center">
                                            Submitted
                                        </div>
                                        <div x-show="item.assignmentsubmitbystudnet === 0"
                                            class="rounded p-3 bg-warning/10 text-warning mt-3 text-center">
                                            Not Submitted
                                        </div>
                                    </td>
                                    <td>
                                        <div class="mt-3 flex gap-8 items-center justify-center">
                                            ...
                                            <svg x-show="!isExpired(item.end_date)" style="cursor:pointer;" width="24"
                                                height="24" viewBox="0 0 24 24" fill="none"
                                                xmlns="http://www.w3.org/2000/svg" @click="handleUpload(item.id)">
                                                <path d="M12 17L12 10M12 10L15 13M12 10L9 13" stroke="#1C274C"
                                                    stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                <path d="M16 7H12H8" stroke="#1C274C" stroke-width="1.5"
...
<!-- Datatables Js -->
<script src="{{ asset('public/assets/js/gernelsettingdata/datatablestudentassignment.js') }}"></script>
    ...
    function myComponent() {
        return {
            handleDownload(assignmentId) {
                var baseUrl = '{{ env("APP_URL") }}/students';
                console.log('Assignment ID:', assignmentId);
                window.open(`${baseUrl}/studentviewassignment/${assignmentId}`, '_blank');// Redirect to the download URL
            },
            handleUpload(assignmentId) {
                var baseUrl = '{{ env("APP_URL") }}/students';
                console.log('Assignment ID:', assignmentId);
                window.open(`${baseUrl}/studentuploadassignment/${assignmentId}`, '_blank');// Redirect to the download URL
            }
        }
    }
</script>
