...
<div class="flex flex-col gap-5 min-h-[calc(100vh-188px)] sm:min-h-[calc(100vh-204px)]">
        ...
        <div x-data="modals">
                 @foreach($user->permissionss as $role)
                    @foreach($role->permissions as $permission)
                        @if($permission->name == 'can_add' && $permission->description == 'Can Add Batch')
                ...
                <div class="flex items-start justify-center min-h-screen " @click.self="open = false">
                    <div x-show="open" x-transition x-transition.duration.300
                        ...
                        <div
                            ...
                            <h5 class="font-semibold text-lg">Add Batch</h5>
                        ...
                        <form action="{{ route('batchcreate') }}" method="POST" id="batchForm">
                        @csrf
                            <div class="text-lightgray text-sm font-normal">
                                <div class="mt-16 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4">
                                    <div class="p-1">
                                        <h2 class="text-base font-semibold mb-4">Name</h2>
                                        <input id="batchname" name="batchname" type="text" class="form-input w-full" placeholder="Name">
                                    </div>
                                    <div class="p-1">
                                        <h2 class="text-base font-semibold mb-4">Code</h2>
                                        <input id="batchcode" name="batchcode" type="text" class="form-input w-full"
                                            placeholder="Code">
                                    </div>
                                    <div class="p-1">
                                    ...
                                    <div class="p-1">
                                        ...
                                        <label class="inline-flex items-center">
                                            <input type="radio" name="status" id="status" class="form-radio text-primary"
                                                checked="">
   ...
   <div x-data="{ openIndex: null }">
    <div class="border rounded-md overflow-hidden">
     <!-- Accordion Item 1 -->
     <div class="border-b"> <button type="button" @click="openIndex === 0 ? openIndex = null : openIndex = 0" class="w-full flex justify-between items-center p-4 focus:outline-none text-gray-500 focus:text-orange-500"> <span>Tag Years to this Batch</span> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 transform transition-transform" :class="{ 'rotate-45': openIndex === 0 }">
    ...
    <div class="w-full md:w-1/2 mb-4 md:mb-0">
        <h2 class="text-lg font-semibold mb-2 text-center md:text-left">Available Year</h2>
        <table class="bg-white border rounded p-4 max-h-64 overflow-y-auto">
            ...
            <tbody id="available-venues">
                @foreach ($yearsWithPrograms as $year)
                <tr class="hover:bg-gray-100" 
                data-year-id="{{ $year['id'] }}" 
                data-program-id="{{ $year['program_id'] }}">
    <td class="text-center">{{ $year['name'] }}</td>
    <td class="text-center">{{ $year['program'] }}</td>
    ...
    <div class="w-full md:w-1/2" style="margin-left: 25px;">
        <h2 class="text-lg font-semibold mb-2 text-center md:text-left">Selected Year</h2>
        <table class="bg-white border rounded p-4 max-h-64 overflow-y-auto">
            <thead style="background-color: #7780a11a;">
                <tr>
                    <th>Name</th>
                ...
                <div class="overflow-auto">
                    <table class="min-w-[640px] w-full">
                        ...
                        <template x-for="(batch, index) in batches" :key="index">
                                <template x-for="(info, infoIndex) in batch.additional_info" :key="infoIndex">
                                    <tr x-show="checkView(index + 1)">
                                        <td>
                                            <span x-text="info.program_name"></span>
                                        </td>
                                        <td>
                                            <span x-text="info.year_name"></span>
                                        </td>
                                        <td>
                                            <span x-text="batch.name"></span>
                                        </td>
                                        <td>
                                            <span x-text="batch.code"></span>
                                        </td>
                                    </tr>
                                </template>
                            </template>
...
@section('script')
<script>
    window.batches = @json($batchDetailsData);
    console.log(window.batches); // Debug to ensure data is being passed correctly
</script>
<!-- Datatables Js -->
<script src="{{ asset('public/assets/js/gernelsettingdata/datatablebatch.js') }}"></script>
...
document.getElementById('batchForm').addEventListener('submit', function () {
        ...
        document.querySelectorAll('#selected-venues tr').forEach(function (row) {
            let yearId = row.getAttribute('data-year-id');
            let programId = row.getAttribute('data-program-id');
            selectedData.push({ year_id: yearId, program_id: programId });
        });
